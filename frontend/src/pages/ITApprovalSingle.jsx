...
import { CheckCircle2, XCircle, Monitor, ShieldCheck, Mail, Zap, User, Building2, Briefcase, ExternalLink, ArrowLeft } from 'lucide-react'
...
export default function ITApprovalSingle() {
  ...
  useEffect(() => {
    const fetchEmployee = async () => {
      try {
        ...
        toast.error('Failed to load employee details for IT approval')
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleITApprove = async (approved) => {
    setProcessing(true)
    try {
      await employeeApi.approveIT(emp.id, {
        approved,
        approverEmail: user?.email || 'it@company.com',
        reason: approved ? 'Approved by IT' : 'Rejected by IT',
      })
      toast.success(approved
        ? '✅ IT Provisioning Approved! Workflow will continue.'
        : '❌ Rejected. Workflow terminated.')
      
      // Update local state to reflect it's no longer pending
      setEmp(prev => ({ ...prev, workflow_status: approved ? 'COMPLETED_STEP' : 'REJECTED' }))
      
      setTimeout(() => {
        navigate('/it-approvals')
      }, 1500)
    } catch (err) {
  ...
  if (!emp) {
    return (
      <div style={{ textAlign: 'center', padding: '80px 20px', color: 'var(--text-muted)' }}>
        ...
        <Button variant="secondary" onClick={() => navigate('/it-approvals')}>Back to Dashboard</Button>
      </div>
    )
  }

  const isPending = emp.workflow_status === 'WAITING_SIGNAL' && emp.current_step_index === 3
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ fontWeight: 700, fontSize: 18, color: 'var(--text)' }}>
            IT Provisioning <span style={{ fontWeight: 400, color: 'var(--text-muted)', fontSize: 14 }}>| Approval</span>
          </div>
        </div>
        
        <div style={{ display: 'flex', gap: 12 }}>
          <Button variant="ghost" size="sm" onClick={() => navigate('/it-approvals')}>All Approvals</Button>
        </div>
      </header>

      <div style={{ padding: '40px 20px', maxWidth: 800, margin: '0 auto' }} className="animate-in">
        <Button variant="ghost" size="sm" icon={<ArrowLeft size={14} />} onClick={() => navigate('/it-approvals')} style={{ marginBottom: 20 }}>
          Back to list
        </Button>

        <PageHeader
          title="Review IT Request"
          subtitle="Please review the provisioning request for the new employee."
        />
            ...
            <div style={{ flex: 1, minWidth: 300 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                {[
                  ...
                  { icon: Monitor, label: 'Requested Laptop', value: emp.laptop_model || 'TBD' },
